...
def preprocess_image(image):
    img = np.array(image) # represents the image (each pixel) into an array
    img = cv2.resize(img, (224, 224))
    img = preprocess_input(img)
    img = np.expand_dims(img, axis=0) # adds another dimension to an img so it creates a list of one img (wrapping img in a list)
    return img
        ...
        if btn:
            with st.spinner("Analyzing Image..."):
                ...
                if predictions:
                    ...
                    for _, label, score in predictions: # (index, label, and the score) (0, "car", 0.9)
